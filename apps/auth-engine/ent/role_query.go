...
// QueryOrgMembers chains the current query on the "org_members" edge.
func (rq *RoleQuery) QueryOrgMembers() *OrgMemberQuery {
	query := (&OrgMemberClient{config: rq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (rq *RoleQuery) Clone() *RoleQuery {
	...
	return &RoleQuery{
		...
		withOrgMembers:  rq.withOrgMembers.Clone(),
...
// WithOrgMembers tells the query-builder to eager-load the nodes that are connected to
// the "org_members" edge. The optional arguments are used to configure the query builder of the edge.
func (rq *RoleQuery) WithOrgMembers(opts ...func(*OrgMemberQuery)) *RoleQuery {
	query := (&OrgMemberClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rq.withOrgMembers = query
	return rq
}
...
func (rq *RoleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Role, error) {
	...
	if query := rq.withOrgMembers; query != nil {
		if err := rq.loadOrgMembers(ctx, query, nodes,
			func(n *Role) { n.Edges.OrgMembers = []*OrgMember{} },
			func(n *Role, e *OrgMember) { n.Edges.OrgMembers = append(n.Edges.OrgMembers, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *RoleQuery) loadOrgMembers(ctx context.Context, query *OrgMemberQuery, nodes []*Role, init func(*Role), assign func(*Role, *OrgMember)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Role)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(orgmember.FieldRoleID)
	}
	query.Where(predicate.OrgMember(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(role.OrgMembersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
