...
func (oq *OrganizationQuery) QueryMembers() *OrgMemberQuery {
	query := (&OrgMemberClient{config: oq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (oq *OrganizationQuery) QueryInvitations() *OrgInvitationQuery {
	query := (&OrgInvitationClient{config: oq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (oq *OrganizationQuery) WithMembers(opts ...func(*OrgMemberQuery)) *OrganizationQuery {
	query := (&OrgMemberClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (oq *OrganizationQuery) WithInvitations(opts ...func(*OrgInvitationQuery)) *OrganizationQuery {
	query := (&OrgInvitationClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (oq *OrganizationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Organization, error) {
	...
	if query := oq.withMembers; query != nil {
		if err := oq.loadMembers(ctx, query, nodes,
			func(n *Organization) { n.Edges.Members = []*OrgMember{} },
			func(n *Organization, e *OrgMember) { n.Edges.Members = append(n.Edges.Members, e) }); err != nil {
			return nil, err
		}
	}
	if query := oq.withInvitations; query != nil {
...
func (oq *OrganizationQuery) loadMembers(ctx context.Context, query *OrgMemberQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *OrgMember)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(orgmember.FieldOrganizationID)
	}
...
func (oq *OrganizationQuery) loadInvitations(ctx context.Context, query *OrgInvitationQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *OrgInvitation)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Organization)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(orginvitation.FieldOrganizationID)
	}
