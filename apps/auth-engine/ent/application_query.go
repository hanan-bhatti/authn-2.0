...
func (aq *ApplicationQuery) QueryTenant() *TenantQuery {
...
func (aq *ApplicationQuery) QueryAPIKeys() *ApiKeyQuery {
...
func (aq *ApplicationQuery) WithTenant(opts ...func(*TenantQuery)) *ApplicationQuery {
	query := (&TenantClient{config: aq.config}).Query()
...
func (aq *ApplicationQuery) WithAPIKeys(opts ...func(*ApiKeyQuery)) *ApplicationQuery {
	query := (&ApiKeyClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(application.FieldTenantID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Application.Query().
//		Select(application.FieldTenantID).
//		Scan(ctx, &v)
...
func (aq *ApplicationQuery) loadTenant(ctx context.Context, query *TenantQuery, nodes []*Application, init func(*Application), assign func(*Application, *Tenant)) error {
...
func (aq *ApplicationQuery) loadAPIKeys(ctx context.Context, query *ApiKeyQuery, nodes []*Application, init func(*Application), assign func(*Application, *ApiKey)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(apikey.FieldApplicationID)
	}
