...
func (akq *ApiKeyQuery) QueryApplication() *ApplicationQuery {
	query := (&ApplicationClient{config: akq.config}).Query()
...
func (akq *ApiKeyQuery) Clone() *ApiKeyQuery {
	...
	return &ApiKeyQuery{
		...
		withApplication: akq.withApplication.Clone(),
...
func (akq *ApiKeyQuery) WithApplication(opts ...func(*ApplicationQuery)) *ApiKeyQuery {
	query := (&ApplicationClient{config: akq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	akq.withApplication = query
	return akq
}
...
//
//	client.ApiKey.Query().
//		GroupBy(apikey.FieldApplicationID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.ApiKey.Query().
//		Select(apikey.FieldApplicationID).
//		Scan(ctx, &v)
...
func (akq *ApiKeyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*ApiKey, error) {
	...
	if query := akq.withApplication; query != nil {
		if err := akq.loadApplication(ctx, query, nodes, nil,
			func(n *ApiKey, e *Application) { n.Edges.Application = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (akq *ApiKeyQuery) loadApplication(ctx context.Context, query *ApplicationQuery, nodes []*ApiKey, init func(*ApiKey), assign func(*ApiKey, *Application)) error {
...
func (akq *ApiKeyQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := akq.ctx.Fields; len(fields) > 0 {
		...
		if akq.withApplication != nil {
			_spec.Node.AddColumnOnce(apikey.FieldApplicationID)
		}
	}
