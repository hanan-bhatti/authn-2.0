...
| **Cross-Site Request Forgery (CSRF)** | Malicious origin triggers unauthorized session actions. | **Server-Side Single-Use OAuth State**: Social auth flows enforce a `state` parameter of 16 cryptographically random bytes, hex-encoded to 32 characters, persisted as a `social_auth_states` row rather than a client-held cookie. The row is hard-deleted on read before the expiry check, so a replayed callback URL fails whether or not it had expired. Custom client API routes require `X-Authn-Publishable-Key` headers. |
...
| **Multi-Tenant Data Leakage** | Tenant A reads, edits or deletes Tenant B's users or sessions, typically by naming a foreign row ID in a request. | **Ent Privacy Interceptors (Reads and Writes)**: Interceptors run beneath the query builder, injecting `WHERE tenant_id = ? AND environment = ?` into every read *and* into every update and delete, from one shared predicate definition per entity so the two directions cannot diverge. Entities without their own `tenant_id` are narrowed through the parent that has one. Creates, having no row to match, are stamped with the caller's tenant, and a create naming a different tenant is refused rather than rewritten. A write against a foreign ID matches nothing and surfaces as `404 Not Found`, indistinguishable from an unknown ID. **Fail-Closed, Twice**: an operation with no tenant in context is refused rather than run unfiltered, and an entity type with no scoping rule is refused rather than allowed through — so a newly added schema type breaks loudly on first use instead of silently spanning tenants. |
