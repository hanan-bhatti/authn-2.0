...
- **`expires_in` always matches the signed `exp`.** The examples in this document show `900` because they are written against the deployment default. A tenant that set `access_token_ttl_minutes` sees its own figure, and against a test credential the same field reports the clamped one — because the advertised lifetime and the `exp` stamped into the token are resolved by one call at each issuance site. A client that schedules its next refresh from `expires_in` therefore stays correct in both environments without knowing which it is in.
